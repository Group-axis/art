...
object DateMapper {
  implicit val time : java.sql.Timestamp = new java.sql.Timestamp(DateTime.now().getMillis)
  implicit val date : DateTime = DateTime.now()
  implicit val timeOption : Option[java.sql.Timestamp] = Some(new java.sql.Timestamp(DateTime.now().getMillis))
  implicit val dateOption : Option[DateTime] = Some(DateTime.now())

  import slick.driver.PostgresDriver.api._

   val utilDate2SqlTimestampMapper = PostgresDriver.MappedColumnType.base[DateTime, java.sql.Timestamp](
 ...
 sqlTimestamp => new DateTime(sqlTimestamp.getTime) )
   ...
   val utilDate2SqlTimestampMapperOption =
     PostgresDriver.MappedColumnType.base[Option[DateTime], java.sql.Timestamp](
...
//     sqlTimestamp => sqlTimestamp.map(time => new java.util.Date(time.getTime)).orElse(None) )
    d => new Timestamp(d.map(_.getMillis).getOrElse(DateTime.now().getMillis)),
    time => Some(new DateTime(time.getTime)))

    val utilDate2SqlDate = PostgresDriver.MappedColumnType.base[DateTime, java.sql.Date](
 utilDate => new java.sql.Date(utilDate.getMillis) ,
...
case class UserDetailEntity(
             ...
             resetDate : Option[DateTime],
             lastConnectionDate : Option[DateTime],
             pwd: Option[String],
             lockStatus : Char,
             failedConnection : Int) {
