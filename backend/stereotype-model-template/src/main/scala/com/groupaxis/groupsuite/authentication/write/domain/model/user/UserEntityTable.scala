...
* */
  * */
  ...
  class Users(tag: Tag) extends Table[UserEntity](tag, "SYS_USER") {
    ...
    def creationDate = column[Option[Date]]("DATE_CREATION")//(DateMapper.utilDate2SqlTimestampMapperOption)
    def userModificationId = column[Option[String]]("ID_USER_MODIFICATION")
    def modificationDate = column[Option[Date]]("DATE_MODIFICATION")//(DateMapper.utilDate2SqlTimestampMapperOption)


    private type UserEntityTupleType = (String, String, String, Option[String], Char, Option[String], Option[Date], Option[String], Option[Date])
...
class UserDetailDAO(val driver: JdbcProfile) {
  ...
  class UserDetails(tag: Tag) extends Table[UserDetailEntity](tag, "SYS_USER_DETAIL") {
    ...
    def resetDate = column[Option[Date]]("DATE_RESET")//(DateMapper.utilDate2SqlTimestampMapperOption)
    def lastConnectionDate = column[Option[Date]]("LAST_CONNECTION_DATE")//(DateMapper.utilDate2SqlTimestampMapperOption)
    ...
    private type UserDetailEntityTupleType = (String, Option[String], Option[Date], Option[Date], Option[String], Char, Int)
