...
trait FeedbackDtnCpyDAO extends DBDriver {

  // Import the query language features from the driver
  import driver.api._

  protected class AMHFeedbackDistributionCps(tag: Tag) extends Table[AMHFeedbackDistributionCpyEntity](tag, "AMH_FB_DT_CP_SEL_TAB") with TableAudit {
  ...
  val amhFeedbackDistributionCps : TableQuery[AMHFeedbackDistributionCps] = TableQuery[AMHFeedbackDistributionCps]

}

object FeedbackDtnCpyDAO extends FeedbackDtnCpyDAO {
  def apply(driver: JdbcProfile) = FeedbackDtnCpyDAO
}

trait FeedbackDtnCpyRuleDAO extends DBDriver {

  import driver.api._

  protected class AMHFeedbackDtnCpyRules(tag: Tag) extends Table[AMHFeedbackDistributionCpyRuleEntity](tag, "AMH_FB_DT_CP_SEL_TAB_RU") with TableAudit {
    def code = column[String]("CODE", O.PrimaryKey)
  ...
  val amhFeedbackDtnCpyRules = TableQuery[AMHFeedbackDtnCpyRules]

}

object FeedbackDtnCpyRuleDAO extends FeedbackDtnCpyRuleDAO {
  def apply(driver: JdbcProfile) = FeedbackDtnCpyRuleDAO
}

trait FeedbackDtnCpyBackDAO extends DBDriver {

  import driver.api._

  protected class AMHFeedbackDtnCpyBackends(tag: Tag) extends Table[AMHFeedbackDistributionCpyBackendEntity](tag, "AMH_FB_DT_CP_SEL_TAB_MFD") with TableAudit {
    def code = column[String]("CODE", O.PrimaryKey)
  ...
  val amhFeedbackDtnCpyBackends = TableQuery[AMHFeedbackDtnCpyBackends]

}

object FeedbackDtnCpyBackDAO extends FeedbackDtnCpyBackDAO {
  def apply(driver: JdbcProfile) = FeedbackDtnCpyBackDAO
}